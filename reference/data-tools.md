...
資料工具用於資料匯入、隔離展示資料與完整 Snapshot 交付。一般啟動流程只需從 [QMAH-Database](https://github.com/MSIT173-03/QMAH-Database) 取得相容的 `QMAH.sql`，不必開啟本目錄的工具，也不必手動執行增量 SQL。
...
目前 exporter 預設寫入 sibling Repository 的 `QMAH-Database/QMAH.sql`。若目標 Repository 或指定資料夾不存在，腳本會在開始時直接停止，避免誤把大型 Snapshot 寫回產品程式 Repository。
