...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 一條流程可能同時碰到文物、遊戲、會員和資產，但「會被使用」不等於「可以直接修改」。先劃出資料主責，才能避免各 Area 各做一份規則，或為了顯示方便直接改寫別人的資料。 |
| What（現在實際怎麼分） | `Catalog` 負責文物與解鎖，`Game` 負責房間與結算，`Social` 負責內容與活動，`User` 負責會員與 Identity，`Store` 負責商品與訂單；跨 Area 以 `Id`、外鍵、目前登入身分或明確 Service 連接，Operations 只作統計與管理入口。 |
| How（遇到跨系統工作怎麼走） | 先列出每張會讀寫的資料表，再標出主責、操作者、狀態轉換和歷史保留方式；只讀資料直接依主責查詢，跨表寫入交給共同 Service 並確認交易邊界。最後用未登入、無權限、錯誤外鍵、重複資料與部分失敗情境檢查邊界是否真的成立。 |

**適用情境：** 一項工作同時碰到兩個以上 Area，或不確定某個欄位應該由 Catalog、Game、Social、User、Store 還是 Operations 修改時，先用本頁畫出資料主責，再決定是直接查詢、呼叫 Service 或只保存外鍵。
