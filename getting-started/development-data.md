...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 同一段程式在不同本機資料上可能得到不同結果，尤其是帳號、文物、題庫、流水和訂單狀態。先使用共同 Snapshot，才能判斷問題是程式、資料庫還原或個人測試資料造成的。 |
| What（現在實際怎麼做） | QMAH-Database 的 `db-v0.8.0` 提供共同展示資料；網站只連到還原後名為 `QMAH` 的 SQL Server 資料庫，啟動時不建表、不清空，也不自動執行 seed。完整資料表、帳號、資產流水、活動和展示關聯以 Snapshot 內容為準。 |
| How（換電腦或重建資料怎麼走） | 先取得相容的 `QMAH.sql` 或已驗證 `.bak`，還原到本機 `QMAH`，確認連線與 Schema，再啟動 API／Web。若要改展示資料，使用資料工具在隔離資料庫產生後重新輸出 Snapshot，不直接在共同資料庫留下無法說明的手動資料。 |

**適用情境：** 換電腦、SSMS 看得到資料但網站查不到、需要把本機資料恢復成組員共用狀態，或要確認展示會員／流水是否來自同一版本時，先依本頁還原 `db-v0.8.0`，再排查程式連線。
