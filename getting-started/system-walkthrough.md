...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 跨系統問題通常不是某一張表少一欄，而是請求入口、登入身分、狀態轉換、交易或流水沒有接起來。先看完整流程，才能知道每個資料表在前後哪一段發揮作用。 |
| What（現在實際串起什麼） | 本頁用一次 HTTP request、文物解鎖、主遊戲作答／投票／結算、優惠券、使用者登入活動和營運批次資產活動，說明 Catalog、Game、Social、Store、User、Operations 與 Shared 如何互相協作。每段都指出目前狀態、主責資料與可回查的流水。 |
| How（遇到一條流程怎麼查） | 先看「目前專題狀態」與「一分鐘快速查閱」，再選最接近的流程段落；沿著「誰發起、送出什麼、哪個 Service 判斷、寫入哪些表、回應什麼」逐步核對。需要精確欄位、狀態碼或限制時，再回到 REST API、資料表參考和共用服務文件。 |

**適用情境：** 只看單一 Controller 仍無法解釋登入、解鎖、遊戲獎勵、優惠券或批次資產結果時，用這頁沿著一次完整請求和資料流回查，找出真正改變狀態的 Service 與流水。
