...
每個 Domain 由自己的分支開始，Component 的 TypeScript、HTML、SCSS 與使用它的 service 放在同一個功能附近；SCSS 可直接寫普通 CSS。既有功能維持目前 route 與 App Shell，新增入口仍由整合者統一接進 `app.routes.ts`，避免五個人同時修改根路由檔案。
