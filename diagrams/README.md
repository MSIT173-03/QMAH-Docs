...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 文件站看到的 SVG 只是產物，圖表內容還有來源檔與可編輯檔。直接在 SVG 上修線或文字，下一次產出就會被覆蓋，文件引用也可能和原始資料分離。 |
| What（現在實際怎麼分） | `.json` 保存 Diagram IR（圖表中介格式）、節點內容、座標與連線；`.drawio` 提供 draw.io Desktop 編輯；`rendered/*.svg` 是 VitePress 顯示用輸出。三者用途不同，真正需要保存的修改應回到來源檔。 |
| How（要改一張圖怎麼走） | 先在 `.json` 修改資料與連線，再產生 `.drawio` 並輸出 `rendered/*.svg`，接著檢查 Markdown 引用的檔名、圖例、替代文字和連線方向。若只收到 SVG，先找回對應 IR，不把輸出檔當成唯一來源。 |

**適用情境：** 要更新系統架構圖、資料庫關聯圖或 Snapshot 流程圖，或發現文件站的圖和文字不一致時，先回到本頁找來源檔與產出流程，再重新輸出網站使用的 SVG。
