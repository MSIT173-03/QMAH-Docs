...
資料庫只保存 `/media/...` 或 `/uploads/...` 形式的邏輯路徑。`QmahMediaUrlResolver` 讀取 `Media` 設定，在輸出 DTO 或 Razor 圖片標籤時轉成本機或 CDN 網址。因此更換檔案來源時，不需要批次改寫文物、商品或會員資料。

目前版本的共用設定固定使用 `Local`，供本機開發與測試。Azure 或 Cloudflare 的設定只作為未來測試環境、預備環境與正式環境的部署設定。

Azure 或 Cloudflare 的值應放在對應環境設定或平台環境變數，不寫入本機預設設定檔。
