...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 五個功能系統可以平行做自己的前台，但登入、錯誤、分頁、圖片、地點和跨系統識別碼如果各做一套，最後會出現同一種狀態有不同顯示或不同重試方式。這頁把可以共用的前台責任和各系統自己的責任分開。 |
| What（現在實際怎麼分） | 一個功能由 route、component、service、state 和 API 組成；共用層處理 HTTP、登入、錯誤、分頁、媒體與地點狀態，各系統元件只處理自己的畫面與流程。Game、經濟、社群活動等跨系統功能仍要以 API 回應與識別碼串起來。 |
| How（接手一個功能怎麼走） | 先找出該功能的 API、資料主責和跨系統識別碼，畫出成功與失敗狀態，再完成最小的載入、顯示、送出和回讀流程。之後補空資料、權限、驗證、重複送出、分頁、圖片／地點失敗與結算狀態，最後檢查共用服務沒有被功能元件重複實作。 |

**適用情境：** 多位組員要同時開發 User、Catalog、Social、Game、Store 或跨系統經濟功能時，用這頁先分配 route、component、service 和共用層責任，再確認各功能交會的 API、識別碼和狀態。
