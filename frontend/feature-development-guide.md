...
UI 預設直接使用 Tailwind CSS 與 daisyUI，Component 預設產生 `.scss`；SCSS 可直接寫普通 CSS，特殊視覺再放進 component SCSS，不自行加入另一套 theme 或完整 UI framework。`src/styles.css` 維持 Tailwind CSS 與 daisyUI 的全域入口，不要在 Component SCSS 重複引入它們。日常範例與規則只維護在 [Angular 使用者前台開發：UI 與樣式](angular-development.md#ui-and-styling)。
...
5. **接上 Page**：在 Domain 下的功能目錄建立 standalone component，TS／HTML／SCSS 放一起，inject service；一般 UI 使用 template 的 Tailwind／daisyUI，特殊視覺再寫入 component SCSS。模板需要的 pipe／component 明確加入 imports。用 signal 保存互動狀態，`finalize` 解除 loading。範例見前面的可選測試頁，正式 UI 同樣沿用該 service。
