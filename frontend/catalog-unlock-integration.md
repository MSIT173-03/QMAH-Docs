...
本頁供圖鑑前端開發者實作「點解鎖 → 選鑰匙 → 確認 → 顯示收藏卡」。後端負責驗證範圍、扣鑰匙與保存解鎖；前端負責選擇、顯示結果與刷新資料。契約核對日期：2026-09-21；程式已建置驗證，尚未完成實際資料庫端到端測試。
...
從背包選鑰匙後，可直接查 GET `/api/v1/me/keys/{keyCode}/artifacts?q=&page=1&pageSize=20`。後端共用實際解鎖的候選規則，自動限定分類／年代、排除已解鎖與停用文物；回應是既有 `ApiPage<ArtifactListItemDto>`，包含縮圖網址。`q` 搜尋名稱或文物編號；切換鑰匙或搜尋條件時回第一頁。

沒有候選回傳空分頁；鑰匙不存在或停用回傳 404。餘額為零仍能預覽範圍，是否可按確認仍依 `/me/economy` 的 balance 決定。一般鑰匙的列表只供預覽隨機抽選範圍，不能點文物指定解鎖。

完整順序是「定義／背包 → 選 key.code → 查該鑰匙 artifacts → 選 artifact.id → POST unlock → 刷新背包與圖鑑」。候選查詢不扣鑰匙、不預留文物，送出時後端再次驗證。
