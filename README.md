...
這個 Repository 是 QMAH 的正式文件來源，也是部署到 GitHub Pages 的 VitePress 文件站來源。文件保留在 Repository 中供檢視、版本追蹤與離線閱讀；文件站直接以同一批 Markdown 建置，不維護另一份網站專用內容。

## 文件閱讀順序

文件採用「先建立共同基線，再進入系統，再查閱契約與交付規則」的順序。首頁提供工作索引；下列路線則列出完整閱讀脈絡：
...
4. 依功能範圍開啟 [六系統快速查詢索引](index.md#system-index)，再進入該頁列出的正規文件。
5. 需要精確欄位、HTTP 行為、工具參數或交付規則時，回到 [參考文件](reference/rest-api.md) 分類查詢。

## 六系統快速查詢

快速查詢頁只負責把一個系統需要的入口、契約、資料、畫面與檢查點聚在一起；詳細規則仍以各頁連結的正規文件為準。
...
檔案依「要解決的問題」命名，不依歷史文件編號命名。每一份系統文件應在開頭說明本頁負責與不負責的範圍；跨系統內容只保留一個正規來源，其他頁面以連結引用。
...
- Repository 內的 Markdown 是可 review、可 clone、可在變更中一起提交的原始文件。
- VitePress 直接讀取這些 Markdown，提供側欄、全文搜尋與適合瀏覽器閱讀的網址。
