...
完成時前台送出原始分數與結果 JSON 文字。伺服器檢查分數介於 0～100，並依玩法驗證盤面後計算標準化分數：館藏拼圖為 5×5 共 25 塊，長卷復位為 3×5 共 15 段，館藏翻牌標準為 16 張牌、8 組配對；素材池不足時依實際素材數量縮減。Grade（評級）、點數及鑰匙進度獎勵則依資料庫設定計算，前台不能指定 `S` 或獎勵數量。`rawResultJson` 最多 4,000 字元，非空時須能解析為 JSON 物件或陣列；DTO 與服務都保留長度檢查。
