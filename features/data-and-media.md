...
正式基準包含 512 件文物，分布於 8 類；分類數量依來源品質與去重結果分配：
...
名稱正規化後的 512 件沒有重複群組；錢幣若來源沒有足夠的不同名稱，才允許同名保留最多兩件，並優先選擇外觀差異較大的來源圖。這比每一類硬湊相同數量更能維持瀏覽與猜題的多樣性。

圖鑑可收錄較多文物。遊戲題庫只使用圖片主體清楚、年代能自動對應單一年代桶，而且不需人工確認的資料。

正式 512 件文物均已通過這些規則，並各自建立一筆 `ArtifactQuestionEntries`；同一批資料也各自建立一件文物明信片商品。後續新增資料仍由該表控制是否可出題，不應只用分類判斷。
...
商城基準包含 512 件文物明信片商品，每件文物各對應一件商品；這些是課程展示資料，不是故宮官方商品，也不宣稱是原作文物的複製品。

商品頁的主視覺使用文物 `display.jpg`，商品列表與相關商品則使用同一目錄的 `thumbnail.jpg`。圖片本身不重新繪製，也不因商品而複製第二份大檔。
...
商品列表的 `ProductCard`／`ProductRow` 只讀 `thumbnail.jpg`，商品詳情才讀 `display.jpg` 並載入可翻面的明信片主視覺。詳情頁採原生 CSS 3D、單一低透明度公版紙張平舖材質、紙板邊緣、反光與低幅度指標傾斜，不引入 Three.js；平面明信片沒有必要付出 WebGL runtime 成本，未來若要做真正的立體卡片模型，再以 lazy chunk 單獨引入。正式材質來源與授權記錄在 `QMAH.Client/public/images/materials/postcard/attribution.json`。

明信片正面採滿版印刷；影像長寬小於 0.65 時在詳情展示中旋轉 90 度成橫向構圖，避免長幅畫在橫式明信片只剩窄條。背面保留 QMAH Logo、基本資料、郵票位與地址線，文字以低彩度混合墨色疊在紙紋上，避免變成高亮 UI；翻面軌道與紙卡邊緣分離，避免正反面或厚度層在動畫中穿透。商品列表不顯示這套 3D 互動。
...
商品名稱格式為「文物名稱－文物明信片」。Description 固定依序包含：

1. 依 8 個分類選出的商品開場，帶入文物名稱與年代。
2. 分類、年代、明信片尺寸與原作尺寸（來源未提供時照實標示）。
3. 虛擬展示、非官方且不提供實際交易的課程聲明。
4. 文物圖像的 CC BY 4.0 姓名標示。
5. 原文物說明；若來源沒有說明則清楚標示。

`Artifacts.SizeText` 保存官方尺寸文字；`Products.SizeText` 固定保存 A6 明信片尺寸，不把文物尺寸誤當成商品尺寸。原作尺寸由商品詳情另外回傳；官方資料寫「待測量」或未提供時照實標示，不產生推測數字。明信片可留作收藏，也可在背面寫短訊息寄給親朋好友，實際寄送依當地郵務規定辦理。
...
優惠活動不另建一張與優惠券重複的活動表。`QmahDatabaseRelease generate-showcase-data` 會依目前啟用且尚未結束的 `CouponDefinitions` 產生官方商城公告，並放在 `SocialPosts` 的 `STORE` 看板；這些公告是展示用的假資料，內容會帶出優惠名稱、折扣方式、消費門檻、取得方式、活動期間與有效天數。

商城可透過 `GET /api/v1/store/promotions` 直接引用同一則公告。回應的 `title` 可當標語、`content` 可當本文、`publishedAt` 可當日期，前台可以依版位選擇只顯示其中一層，不需要解析或重組公告文字。公告只負責呈現，實際優惠券可否使用仍由結帳時的優惠券定義重新檢查。

官方人員在社群貼文編輯器可以快速插入商城優惠、展覽活動、文物導讀、參觀提醒與鑑定遊戲等區塊。這些區塊不是另一種資料表，而是帶有固定段落標記的可讀純文字；前台依 `【小標】`、`• 項目`、`「引用」` 與空行呈現層次，既保留既有 `SocialPosts.Content` 契約，也避免把 HTML 寫入資料庫。有效優惠券才會出現在商城快速插入選單，沒有優惠券時其他官方模板仍可使用。
