...
正式 512 件文物均已通過這些規則，並各自建立一筆 `ArtifactQuestionEntries`；同一批資料也各自建立一件文物收藏卡商品。後續新增資料仍由該表控制是否可出題，不應只用分類判斷。
...
商城基準包含 512 件文物收藏卡商品，每件文物各對應一件商品；這些是課程展示資料，不是實體縮小複製品，也不是故宮官方商品。
...
商品名稱格式為「文物名稱－文物收藏卡」。Description 固定依序包含：

1. 依 8 個分類選出的商品開場，帶入文物名稱與年代。
2. 分類、年代、收藏卡尺寸與原作尺寸（來源未提供時照實標示）。
3. 虛擬展示、非官方且不提供實際交易的課程聲明。
4. 文物圖像的 CC BY 4.0 姓名標示。
5. 原文物說明；若來源沒有說明則清楚標示。

`Artifacts.SizeText` 保存官方尺寸文字；`Products.SizeText` 保存收藏卡展示需要的尺寸摘要，不把文物尺寸誤稱為複製品尺寸。官方資料寫「待測量」或未提供時照實標示，不產生推測數字。
