...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | 同一件文物會被圖鑑、題庫、遊戲和商品引用，但來源授權、商品欄位和圖片檔案不是同一種資料。把實際網址寫進業務資料，日後從本機切到 CDN 時就必須大量改資料列。 |
| What（現在實際怎麼分） | `catalog.Artifacts` 保存文物主資料與邏輯媒體路徑，`game.ArtifactQuestionEntries` 和 `store.Products` 以 `ArtifactId` 建立關聯；來源與授權欄位留在文物資料，商品定價／庫存和媒體實體檔案各自管理。應用程式依媒體設定把邏輯路徑解析成本機或 CDN 網址。 |
| How（新增、匯入或換圖片來源怎麼走） | 先確認來源網址、授權文字、`ArtifactId` 和檔案是否互相對應，再保存穩定的邏輯路徑；畫面只使用 resolver 產生的 URL。更換儲存位置時搬移檔案、切換設定並驗證本機與公開網址，不批次改寫文物、商品或社群資料的路徑。 |

**適用情境：** 要新增文物圖片、確認 NPM 授權、修正圖片顯示、處理 `altText`，或準備從 Local 搬到 CDN 時，先用本頁確認資料列、實體檔案和公開 URL 的分工，再進入[媒體交付設定](../frontend/media-delivery.md)查環境切換。
