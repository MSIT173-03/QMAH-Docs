...
| 先問自己 | 文件直接回答 |
| --- | --- |
| Why（為什麼要看這頁） | QMAH 需要讓活動、貼文和會員地址可以顯示位置或讓使用者選點，但不需要把某一家地圖服務的資料格式變成業務資料。把地圖服務的識別碼或搜尋結果存進核心表格，未來更換服務時會被綁住。 |
| What（現在實際怎麼做） | QMAH 保存地址文字與可選的 `Latitude`／`Longitude`；後台目前用共用 JavaScript 產生 OpenStreetMap 連結，沒有內嵌圖磚，也不保存地點識別碼或地理編碼結果。活動、貼文和會員地址仍由原本的資料主責保存。 |
| How（串接前台或後台怎麼走） | 先讀 API 回傳的地址文字和座標，再決定要顯示連結、地圖或選點元件；使用者選定後只回寫 QMAH 的地址欄位，沒有座標時保留空值。更換地圖元件只改顯示／選點轉換和外部服務設定，API 的業務欄位與資料主責不改。 |

**適用情境：** 要在活動、貼文或會員地址顯示地圖、加上地址搜尋／選點，或要把後台目前的 OpenStreetMap 連結接到 Angular 前台時，先依本頁確認回傳欄位和空座標行為，不把地圖服務的識別碼存成 QMAH 主資料。
