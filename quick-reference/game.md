...
目前 Mini Game 已具備共用的開始、完成、評級與經濟獎勵流程。完成請求會檢查分數範圍、結果 JSON 格式與各模式的盤面結果；館藏拼圖固定驗證 25 塊、長卷復位固定驗證 15 段，館藏翻牌依素材池驗證最多 8 組配對。
...
3. complete 會檢查分數範圍、JSON 格式與模式盤面結果，再由伺服器計算標準化分數、Grade、點數和鑰匙進度。館藏拼圖使用 5×5（25 塊），長卷復位使用 3×5（15 段），館藏翻牌標準為 16 張牌（8 組），素材數不足時依實際素材池縮減。
...
相同會員重送已完成的 Attempt，且輸入通過格式檢查時，API 回傳 `200` 與 `alreadyCompleted: true`，不再次發獎。找不到該會員的 Attempt 回傳 `404`，其他不可完成狀態回傳 `409`。評級依伺服器驗證後的模式分數和模式門檻計算。
