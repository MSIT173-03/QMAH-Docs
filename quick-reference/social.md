...
圖鑑詳細頁只對已解鎖文物顯示「前往社群討論」。前台先查詢該 `ArtifactId` 是否已有已發布的 `POST`；有就導向最早的 canonical 貼文，沒有就開啟確認視窗，要求會員輸入第一則留言，再由 `POST /api/v1/social/artifacts/{artifactId}/discussion` 在同一交易內建立貼文與留言。建立失敗只回傳該功能的錯誤狀態，不阻斷圖鑑或其他 API。
