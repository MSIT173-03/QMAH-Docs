...
目前 `db-v0.9.3` 基準有 512 件文物、18 個年代桶與 512 筆題庫；商城對應 512 件文物明信片。新增的 `JAPAN_EDO` 年代有對應的 `KEY-ERA-JAPAN_EDO` 鑰匙，並納入會員餘額與流水；`catalog.ArtifactUnlocks` 也保留可沿 `KeyTransactionId` 查帳的展示解鎖紀錄。詳情頁的文物可從已解鎖內容進入「文物討論」；沒有貼文時，前台會先詢問會員並要求留下第一則留言，才建立該文物的 canonical 討論入口。
